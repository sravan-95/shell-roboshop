...
mysql -h $MYSQL_HOST -u root -pDevOps123 -e "use cities" &>>$LOGS_FILE
if [ $? -ne 0 ]; then
    mysql -h $MYSQL_HOST -uroot -pDevOps123 < /app/db/schema.sql
    mysql -h $MYSQL_HOST -uroot -pDevOps123 < /app/db/app-user.sql
    mysql -h $MYSQL_HOST -uroot -pDevOps123 < /app/db/master-data.sql
